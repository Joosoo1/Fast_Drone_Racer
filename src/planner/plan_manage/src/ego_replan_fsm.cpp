...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(ros::NodeHandle &nh)
  {
    ...
    // subscribe
    odom_sub_ = nh.subscribe("fsm/odom_world", 1, &EGOReplanFSM::odometryCallback, this);

    // 紧急制动
    mandatory_stop_sub_ = nh.subscribe("fsm/mandatory_stop", 1, &EGOReplanFSM::mandatoryStopCallback, this);

    // 实际部署时订阅global_circle_sub_，获取全局轨迹点
    ...
    // publish
    poly_traj_pub_ = nh.advertise<traj_utils::PolyTraj>("fsm/planning/trajectory", 10);
    data_disp_pub_ = nh.advertise<traj_utils::DataDisp>("fsm/planning/data_display", 100);
    heartbeat_pub_ = nh.advertise<std_msgs::Empty>("fsm/planning/heartbeat", 10);
    ground_height_pub_ = nh.advertise<std_msgs::Float64>("fsm/ground_height_measurement", 10);
  ...
  void EGOReplanFSM::execFSMCallback(const ros::TimerEvent &e)
  {
    ...
    case EXEC_TRAJ:
    {
      ...
      else if ((wpt_id_ < waypoint_num_ - 1) && (final_goal_ - pos).norm() < no_replan_thresh_) // case 2: assign the next waypoint
      {
        wpt_id_++;
  ...
  bool EGOReplanFSM::planNextWaypoint(const Eigen::Vector3d next_wp)
  {
    ...
    // one_pt_wps.assign(wps_.begin() + wpt_id_, wps_.end());
    success = planner_manager_->planGlobalTrajWaypoints( odom_pos_, odom_vel_, Eigen::Vector3d::Zero(),
                                                        one_pt_wps, Eigen::Vector3d::Zero(), Eigen::Vector3d::Zero());
                                                        // 获取全局轨迹-traj_.setGlobalTraj(globalMJO.getTraj(), time_now.toSec());
    if (success)
    {
      final_goal_ = next_wp;
      ...
      have_target_ = true;// 状态机进入
      have_new_target_ = true;

      /*** FSM ***/
      if (exec_state_ != WAIT_TARGET)
      {
        // 当不为EXEC_TRAJ时,休眠进程,等待状态机进入EXEC_TRAJ
  ...
  void EGOReplanFSM::globalCircleCallback(const airsim_ros::CirclePoses::ConstPtr& msg)
  {
    if (!this->flag_takeoff_)
    {
      // this->takeoff_client_.call(this->takeoff_);
      // this->flag_takeoff_ = true;
    }
    if(!this->flag_recived_global_goal_)
    {
      this->waypoint_num_ = msg->poses.size();

      for(size_t i = 0; i < msg->poses.size(); ++i)
      {
        waypoints_[i][0] = msg->poses[i].position.x;
        waypoints_[i][1] = -msg->poses[i].position.y;
        waypoints_[i][2] = -msg->poses[i].position.z;
      }
      readGivenWpsAndPlan();
      this->flag_recived_global_goal_ = true;
    }
    else
    {
      return;
  ...
  void EGOReplanFSM::waypointCallback(const quadrotor_msgs::GoalSetPtr &msg)
  {
    ...
    Eigen::Vector3d cur_wp(msg->goal[0], msg->goal[1], msg->goal[2]);

    this->wps_[this->wpt_id_] = cur_wp;
