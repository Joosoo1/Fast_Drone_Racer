...
bool StaticInitializer::initialize(double &timestamp, Eigen::MatrixXd &covariance, std::vector<std::shared_ptr<Type>> &order,
  ...
  Eigen::Matrix3d R_GtoI0_temp = quat_2_Rot(q_GtoI);
  ...
  imu_state.block(0, 0, 4, 1) = rot_2_quat(R_GtoI0_temp * R_yaw.transpose());
  ...
  Eigen::MatrixXd Phi = Eigen::MatrixXd::Identity(t_imu->size(), t_imu->size());
