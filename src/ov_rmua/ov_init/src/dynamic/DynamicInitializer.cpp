...
bool DynamicInitializer::initialize(double &timestamp, Eigen::MatrixXd &covariance, std::vector<std::shared_ptr<ov_type::Type>> &order,
  ...
  Eigen::MatrixXd Phi = Eigen::MatrixXd::Identity(_imu->size(), _imu->size());
  ...
  // Convert the global frame to FLU frame, R_yaw == R_Gov2Gpx4
  Eigen::Matrix3d R_GtoI0_temp = quat_2_Rot(x.block(0, 0, 4, 1));
  auto rpy_angle = rot2rpy(R_GtoI0_temp);
  // std::cout << "rpy_angle(2): " << rpy_angle(2) << std::endl;
  Eigen::Matrix3d R_yaw;
  double theta = -rpy_angle(2, 0);
  R_yaw << std::cos(theta), std::sin(theta), 0, -std::sin(theta), std::cos(theta), 0, 0, 0, 1;
  // std::cout << "R_yaw: \n" << R_yaw << std::endl;
  // std::cout << "R_GtoI0_temp: \n" << R_GtoI0_temp << std::endl;

  Eigen::Vector3d v_I0inG = x.block(7, 0, 3, 1);
  x.block(0, 0, 4, 1) = rot_2_quat(R_GtoI0_temp * R_yaw.transpose());
  x.block(4, 0, 3, 1).setZero();
  x.block(7, 0, 3, 1) = R_yaw * v_I0inG;
  _imu->set_value(x);
  _imu->set_fej(x);
  Phi.block(0, 0, 3, 3) = R_yaw;
  Phi.block(3, 3, 3, 3) = R_yaw;
  Phi.block(6, 6, 3, 3) = R_yaw;
  covariance = Phi * covariance * Phi.transpose();
